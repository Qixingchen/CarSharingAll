...
public class CartakeDAO {
	...
	public List<CarshareDealJSON> selectcartake(String phonenum) {
		...
		SQLQuery query = session
				.createSQLQuery("SELECT * FROM cartake_deal where DriverID=:dphonenum or PassengerID=:pphonenum ORDER BY ID DESC");
		...
		try {
			...
			while (iterator.hasNext()) {
				...
				if ((driver != null && !request[1].toString().equals(
						driver[1].toString()))
						|| driver == null)
		...
		for (int i = 0; i < cartakelist.size(); i++) {
			CarshareDealJSON route = sharedao.selectrouteinfo(cartakelist
					.get(i).getDealId());
	...
	public List<CartakeDealJSON> selectpassnode(String dealid) {
		...
		tx.begin();
		SQLQuery query = session
				.createSQLQuery("SELECT * FROM cartake_deal where DealID=:dealid ORDER BY PassengerOrder");
