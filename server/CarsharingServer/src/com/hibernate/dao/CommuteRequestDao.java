...
public class CommuteRequestDao {
	...
	public List<CommuteRequestJSON> SelectRequest(String phonenum) {
		...
		SQLQuery query = session
				.createSQLQuery("SELECT * FROM commute_request where UserID=:phonenum ORDER BY ID DESC");
	...
	public Boolean saveRequest(HttpServletRequest request) {
		...
		Transaction tx = session.beginTransaction();
		Query query = session
				.createSQLQuery("INSERT INTO commute_request (`UserID`, `RequestTime`, `StartPlaceX`, `StartPlaceY`,`StartPlace`,`DestinationX`,`DestinationY`,`Destination`, `StartDate`, `EndDate`, `StartTime`, `EndTime`, `WeekRepeat`, `SupplyCar`, `DealStatus`) "
		...
		query.setParameter("requesttime",
	...
	public Boolean deleteRequest(String phonenum, String time) {
		...
		Transaction tx = session.beginTransaction();
		Query query = session
				.createSQLQuery("DELETE FROM commute_request WHERE UserID=:phonenum and RequestTime=:time");
		query.setParameter("phonenum", phonenum);
	...
	public Boolean updateRequeststatue(String phonenum, Timestamp time,
			int status) {
		// 创建session
		Session session = HibernateSessionFactory.getSession();
		// 开启事务
		...
		Transaction tx = session.beginTransaction();
		Query query = session
				.createSQLQuery("UPDATE commute_request SET DealStatus=:status WHERE UserID=:phonenum and RequestTime=:time");
		query.setParameter("phonenum", phonenum);
		query.setParameter("time", time);
