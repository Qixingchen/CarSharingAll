...
public class LongwayPublishDao {
	...
	public List<LongwayPublishJSON> selectPublish(String  phonenum)
	{
		...
		tx.begin();
		SQLQuery query=session.createSQLQuery("SELECT * FROM longway_publish where UserID=:phonenum ORDER BY ID DESC");
	...
	public Boolean savePublish(HttpServletRequest request)
	{
		...
		Transaction tx=session.beginTransaction();	
	    Query query=session.createSQLQuery("INSERT INTO longway_publish(`UserID`, `PublishTime`, `UserRole`, `StartDate`, `StartPlace`, `Destination`, `NoteInfo`) VALUES (:phonenum,:publishtime,:userrole,:startdate,:startplace,:destination,:noteinfo)");
	    query.setParameter("phonenum",request.getParameter("phonenum"));   
	    query.setParameter("publishtime",new Timestamp(System.currentTimeMillis()));
	...
	public Boolean deleteRequest(String phonenum,String time)
	{
		...
		Boolean success=true;
		Transaction tx=session.beginTransaction();	
		Query query=session.createSQLQuery("DELETE FROM  longway_publish WHERE UserID=:phonenum and PublishTime=:time");
	    query.setParameter("phonenum", phonenum);
	...
	public List<LongwayPublishJSON> lookuppublish(String  role)
	{
		...
		Transaction tx=session.beginTransaction();
		tx.begin();
		SQLQuery query=session.createSQLQuery("SELECT * FROM longway_publish where UserRole=:role ORDER BY ID DESC");
