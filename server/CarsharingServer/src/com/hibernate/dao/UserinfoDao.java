...
public class UserinfoDao {
	...
	public List<UserInfoJSON> findUserByphonenum(String phonenum) {
		...
		SQLQuery query = session
				.createSQLQuery("SELECT * FROM user_info where PhoneNum=:phonenum");
	...
	public Boolean checklogin(String phonenum, String pwd) {
		...
		tx.begin();
		Query query = session
				.createQuery("from UserInfo where PhoneNum = :phonenum and passWord= :password");
	...
	public Boolean updateUserinfo(HttpServletRequest request) {
		...
		Boolean success = true;
		Transaction tx = session.beginTransaction();
		Query query = session.createSQLQuery("UPDATE user_info SET "
