...
//	String query1="select count(*) as dcount from commute_request where SupplyCar='y'";     //查询有车的人的数量
	String query1 = null;
...
//	ResultSet result1=sql.executeQuery(query1);
	...
	System.out.println("---------------------------------");
	System.out.println("出发地经度"+" "+"出发地纬度"+" "+"目的地经度"+" "+"目的地纬度");
	System.out.println("---------------------------------");
	result.last();
...
/*		PreparedStatement ps=con.prepareStatement("insert into carshare_deal values(?,?,?,?)");     //往carshare_deal中插入数据
		sum=itersum+1;
		ps.setLong(1,sum);
		ps.setString(2,GetNowTime.getTime());
		ps.setString(3,type);
		ps.setLong(4,0);
		ps.executeUpdate();
...
*/		
		...
		query3="Select * from commute_request where DestinationX="+DesX[j]+"and DestinationY="+DesY[j]+"and DealStatus=0";    //get the total information from table group by X,Y
		...
		while(result1.next()){                                //获取每个点的经纬度
			...
			mes=mes+" "+StartPlacelng[num]+" "+StartPlacelat[num]+" ";
			ids=ids+" "+UserID[num];
...
//			System.out.println(StartPlacelng + " " + StartPlacelat + " " + Destinationlng+" "+Destinationlat);
//			Map<String, String> json = maphttp.testPost(StartPlacelng[num],StartPlacelat[num], Destinationlng, Destinationlat, city, city);
//			distance=distance+json.get("address")+" ";
			num++;
		}
//		System.out.println(mes);
		Map<String, String> json1 = province.testcity(Destinationlng,Destinationlat);
		...
		for(int it=0;it<totalPerson[j];it++){           //获取任意两点间的实际距离
			...
			try{
				city1="";
				city2="";
				Map<String, String> json2 = province.testcity(StartPlacelng[it],StartPlacelat[it]);
				city1=city1+json2.get("addre");
				json2 = province.testcity(StartPlacelng[it1],StartPlacelat[it1]);
...
//			warning>>>>>>>>>>>>>>>>
		...
		for(int it=0;it<totalPerson[j];it++){       //获取任意点到目的地的实际距离
			...
			Map<String, String> json = maphttp.testPost(StartPlacelng[it],StartPlacelat[it], Destinationlng, Destinationlat, city3, city);
			distance=distance+json.get("address")+" ";
		}	
//		System.out.println(distance);
//		System.out.println(totalPerson[j]);
//		result.close();
		query1="Select * from commute_request where DestinationX="+DesX[j]+"and DestinationY="+DesY[j]+"and DealStatus=0 and SupplyCar='y'";   //get the number of driver
...
//		sql.executeBatch();
		command=pathname+" "+totalPerson[j]+" "+d+" "+k+" "+DesX[j]+" "+DesY[j]+mes+" "+distance+" "+(itersum+1)+" "+ids;
...
//			 carshare.append(msg.trim());
			 if(msg.equals(st)){
				 PreparedStatement ps1=con.prepareStatement("update commute_request set DealStatus=? where DestinationX="+DesX[j]+"and DestinationY="+DesY[j]+" and DealStatus=0");
				 ps1.setInt(1,1);
				 ps1.executeUpdate();
			 }
